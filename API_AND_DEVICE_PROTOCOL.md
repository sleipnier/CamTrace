...
```http
GET /api/jobs?status=<optional>&limit=20&cursor=<reserved>
```

响应：`{"items": JobDetail[]}`。当前 `cursor` 为兼容保留字段；结果按创建时间倒序。
...
状态：`queued | rendering | succeeded | failed`。
...
跨进程 `.vtc-gpu.lock` 使用 `fcntl.flock`，保证同一 MegaSaM 根目录一次只有一个 GPU 重建任务。标准输出和错误写入任务日志，不作为结构化数据协议。
